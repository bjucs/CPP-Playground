...
void display_warning_output(list<Student_info> students) { 
    ...
    for (auto it = students.begin(); it != students.end(); it++) { 
        try { 
            ...
            cout << "Student grade if HW is not turned in: " << grade(*it) << endl;
        } catch (domain_error) { 
            cout << "Student did not report any homework grades" << endl; 
        }
    }
...
int main() { 
    ...
    if (students.size() != 0) { 
        list<Student_info> missing_hw_students = extract_missing_hw(students);
        display_warning_output(missing_hw_students);
    }
